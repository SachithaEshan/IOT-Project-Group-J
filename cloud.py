...
# Initialize Firebase
cred = credentials.Certificate("C:\\Users\\oshan\\Documents\\iot\\iot-groupj-firebase-adminsdk-hnc2f-3e23d15671.json")
...
alarm_arg = "alarm.wav"
webcam_arg = "http://192.168.162.88/cam-mid.jpg"
            ...
            if COUNTER >= EYE_AR_CONSEC_FRAMES:
                if not ALARM_ON:
                    ...
                    if latitude is not None and longitude is not None and speed is not None:
                        # Log drowsiness event to Firebase
                        ref = db.reference('/drowsiness_events')
                        ref.push({
                            'timestamp': drowsiness_time,
                            'speed': speed,
                            'latitude': latitude,
                            'longitude': longitude
                        })
